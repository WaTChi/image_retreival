...
int alg(string celldir, string cell, string qdir, string qfile,
	...
	flann::Index<int> *index;
	if (fexists(cellpath + indexsuffix)) {
		cout << "loading index" << endl;
		index = new flann::Index<int>(dataset, flann::SavedIndexParams(cellpath
				+ indexsuffix));
	} else {
		cout << "creating index" << endl;
		if (indextype == "kdtree4")
			// construct an randomized kd-tree index using 4 kd-trees
			index = new flann::Index<int>(dataset, flann::KDTreeIndexParams(4));
		else if (indextype == "kdtree1")
			index = new flann::Index<int>(dataset, flann::KDTreeIndexParams(1));
		else if (indextype == "kmeans64,22")
			index = new flann::Index<int>(dataset, flann::KMeansIndexParams(64,
					22));
		else {
