...
image_double makeImageDoubleForPython(unsigned int xsize, unsigned int ysize, double *theData) {
  image_double image;
 ...
 */
double* lsd(image_double image)
{
  /* LSD parameters */

  printf("Hello from LSD in C! \n");

  double scale = 0.8;       /* Scale the image by Gaussian filter to 'scale'. */

  ntuple_list answer = lsd_scale(image, scale);
