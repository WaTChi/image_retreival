...
def computeRanks(dbbase, listofimages, qimgPath):
    ...
    for i, image in enumerate(listofimages):
        ...
        dstring = readTessFile(imagepath, 40)
        
        if i == 0:
            qstring = readTessFile(qimgPath[:-4], 40)
...
def compareTwoImages(img1, img2):
    dstring = readTessFile2(img2, 40);
    qstring = readTessFile2(img1, 40)
...
def returnTopMatch_random(dbbase, listofimages, qimgPath):
    ...
    if topscore <= 1.0:
        return (None, None)
    return (normalizeScores(myDict), max(myDict.iteritems(), key=operator.itemgetter(1))[0])

        
if __name__ == '__main__':
    img1 = '/home/jason/Desktop/query/project/src/tutorial/mall_query/0,0-0040'
    img2 = '/home/jason/Desktop/query/project/src/mall_db/0,0-0076'
    dbbase = 'mall_db'
